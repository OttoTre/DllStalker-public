...
{
	if (const Types::KnownUnityType* known = Types::FindKnownUnityTypeExact(unityType)) {
		return std::string(known->cppType);
	}
...
void CollectEnumTypes(const std::unordered_map<void*, std::vector<FieldInfo>>& fieldsByClass,
					  std::unordered_map<void*, EnumTypeInfo>& out)
{
	for (const auto& entry : fieldsByClass) {
		for (const auto& f : FilterInstanceFields(entry.second)) {
...
std::unordered_map<void*, std::vector<FieldInfo>> BuildFieldCache(
	const std::vector<ClassInfo>& classes, FieldCatalog& fields)
{
	std::unordered_map<void*, std::vector<FieldInfo>> cache;
	for (const auto& klass : classes) {
		if (!klass.klassPtr) {
			continue;
		}
		cache.emplace(klass.klassPtr, fields.GetRawFields(klass.klassPtr));
	}
	return cache;
}
...
SdkExporter::SdkExporter(FieldCatalog& fields, MethodCatalog& methods)
	: m_fields(fields)
	, m_methods(methods)
{
}
...
{
	if (classes.empty()) {
		Engine::Services::BootstrapLog::Write("[SdkExporter] No classes to export.\n");
		return {};
	}
										   ...
										   const std::vector<ClassInfo>& classes)
{
	...
	if (toFile) {
		...
		if (!outFile.is_open()) {
			Engine::Services::BootstrapLog::Write(
				"[SdkExporter] Failed to open '%s' for writing.\n", options.outputPath);
			return {};
		}
	}
	...
	const auto fieldCache = BuildFieldCache(classes, m_fields);

	if (options.includeEnums) {
		std::unordered_map<void*, EnumTypeInfo> enumTypes;
		CollectEnumTypes(fieldCache, enumTypes);
	...
	for (const auto& klass : classes) {
		...
		const auto cacheIt = fieldCache.find(klass.klassPtr);
		if (cacheIt == fieldCache.end()) {
			continue;
		}
		auto relevant = FilterInstanceFields(cacheIt->second);
	...
	if (toFile) {
		outFile.close();
		Engine::Services::BootstrapLog::Write(
			"[SdkExporter] C++ header written to '%s' (%d classes, %d fields).\n",
			   options.outputPath,
			   emitted,
			   totalFields);
	}
		...
		Engine::Services::BootstrapLog::Write(
			"[SdkExporter] C++ header done (%d classes, %d fields).\n", emitted, totalFields);
	}
											 ...
											 const std::vector<ClassInfo>& classes)
{
	...
	if (toFile) {
		outFile.close();
		Engine::Services::BootstrapLog::Write(
			"[SdkExporter] C# stubs written to '%s' (%d classes, %d fields).\n",
			   options.outputPath,
			   emitted,
			   totalFields);
	}
		...
		Engine::Services::BootstrapLog::Write(
			"[SdkExporter] C# stubs done (%d classes, %d fields).\n", emitted, totalFields);
	}
