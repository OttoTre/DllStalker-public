...
std::vector<void*> StaticInstanceFinder::FindStaticInstanceCandidates(void* klass) {
    ...
    for (const uintptr_t staticValue : m_fields.EnumerateStaticFieldAddresses(klass)) {
        if (staticValue == 0) continue;

        void* slotAddress = reinterpret_cast<void*>(staticValue);
