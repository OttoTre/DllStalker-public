...
FieldAnalysisScope BuildFieldAnalysisScope(const ControlPanelSessionState& state) {
...

    {
        ...
        scope.viewInstancePtr = state.inspectorCache.activeInstancePtr;
    }

    scope.breadcrumbCount = state.navigationStack.size();
    if (!state.navigationStack.empty()) {
        const InspectorBreadcrumb& top = state.navigationStack.back();
        scope.inCollection         = top.isCollection;
        scope.collectionFieldName  = top.isCollection ? top.sourceField.name : std::string{};
    }

    scope.breadcrumbLabels.reserve(state.navigationStack.size());
    scope.breadcrumbIsCollection.reserve(state.navigationStack.size());
    for (const auto& step : state.navigationStack) {
