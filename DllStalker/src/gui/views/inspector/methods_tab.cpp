...
void RenderMethodsTab(const InspectorCache& inspectorSnapshot,
    ...
    ImGui::Separator();
    {
        ...
        const int currentInvokeVersion = invokeQueue.latestVersion.load(std::memory_order_acquire);
        if (currentInvokeVersion != s_toast.version) {
            ...
            s_toast.succeeded        = invokeQueue.latestResult.succeeded;
            s_toast.methodName       = invokeQueue.latestMethodName;
            s_toast.returnDisplay    = invokeQueue.latestResult.returnDisplay;
            s_toast.errorMessage     = invokeQueue.latestResult.error;
            invokeQueue.latestAtSeconds = s_toast.stampedAtSeconds;
            // Method audit is recorded in TickPresentSideEffects.
        }
    ...
    if (skipEmptyLoadingTable) {
        ...
        std::vector<size_t> visibleMethodIndices;
        visibleMethodIndices.reserve(inspectorSnapshot.methods.size());
        for (size_t methodIndex = 0; methodIndex < inspectorSnapshot.methods.size(); ++methodIndex) {
            ...
            visibleMethodIndices.push_back(methodIndex);
        }
        visibleMethodCount = visibleMethodIndices.size();

        ImGuiListClipper clipper;
        clipper.Begin(static_cast<int>(visibleMethodIndices.size()));
        while (clipper.Step()) {
            for (int row = clipper.DisplayStart; row < clipper.DisplayEnd; ++row) {
                const size_t methodIndex = visibleMethodIndices[static_cast<size_t>(row)];
                const auto& method = inspectorSnapshot.methods[methodIndex];
                ImGui::TableNextRow();
                if (method.jitFailed) {
